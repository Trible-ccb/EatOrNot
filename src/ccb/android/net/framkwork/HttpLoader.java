...
//		Assert.assertNotNull(task);
		...
		try {
			...
			if ( is == null){
				...
				client.getParams().setIntParameter(HttpConnectionParams.CONNECTION_TIMEOUT, 30 * 1000);
				client.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, 1000* 60);
	...
	public  NetworkInfo getNetworkType(){
		ConnectivityManager conManager = 
			(ConnectivityManager) mContext.getSystemService(Context.CONNECTIVITY_SERVICE);
		NetworkInfo mb = conManager.getActiveNetworkInfo();
		if ( mb != null){
			LogWorker.i("network type = " + mb.getType() + " typename = " + mb.getTypeName() + " subtype= " + mb.getSubtype() +  "subtypename = " + mb.getSubtypeName());
		}
		return mb;
	}
